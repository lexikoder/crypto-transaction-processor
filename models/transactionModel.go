...
type FiatCryptoTransaction struct {
	...
	CryptoAmount *int64  `gorm:"not null;check:crypto_amount > 0" json:"crypto_amount"` // string to  avoid float precision issues
	// Fiat side
	FiatCurrency *Currency `gorm:"not null;check:fiat_currency IN ('NGN','USD')" json:"fiat_currency"`
	FiatAmount   *int64    `gorm:"not null;check:fiat_amount> 0" json:"fiat_amount"`
	CreatedAt    time.Time
	UpdatedAt    time.Time
}